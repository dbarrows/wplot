...
update_all_geom_defaults <- function(dark = FALSE, classic = FALSE) {
    if (dark) {
    ...
    update_geom_defaults('ribbon', list(fill = fill_colour, colour = 'transparent', alpha = 0.15))
    update_geom_defaults('smooth', list(fill = fill_colour, colour = default_colour, alpha = 0.15))
    ...
    update_geom_defaults('rug', list(length = unit(0.02, 'npc'), size = 0.1))
    invisible()
}